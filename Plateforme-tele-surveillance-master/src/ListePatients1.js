...
const ListePatients1 = ({ navigation, route }) => {
  ...
  useEffect(() => {
    async function getAllpatientHs() {
      try {
        const patientHs = await axios.get(`http://192.168.100.198:5000/api/patient/afficher`)
    ...
    <View style={[styles.container, {
      ...
      <View style={[{ flex: 7, backgroundColor: "#FAE7E6", marginLeft: 20, marginRight: 20, marginTop: 15, borderRadius: 20, borderColor: '#FAE7E6' }, { flexDirection: "column" }]}>
        {/* <View style={[styles.bord, { flexDirection: "row" },]}>
          <Text style={{ marginLeft: width * 0.07, marginTop: height * 0.05, fontFamily: 'poppins', color: "#E3807B", fontWeight: 'bold', fontSize: 20 }}>Ajouter Un patientH</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            <TouchableOpacity onPress={() => {
              navigation.navigate("ProfilePatient2", { idpatient: item._id, idUser: donnee })
            }
            }>
