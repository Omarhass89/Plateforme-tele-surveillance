...
const ListeAdmin = ({ navigation, route }) => {
  const deleteadmin = async (id) => {
    await axios.delete(`http://192.168.100.198:5000/api/admin/delete/${id}`)
  }
  ...
  useEffect(() => {
    async function getAllAdmins() {
      try {
        const admins = await axios.get('http://192.168.100.198:5000/api/admin/afficher')
    ...
    <View style={[styles.container, {
      ...
      <View style={[{ flex: 7, backgroundColor: "#FAE7E6", marginLeft: 20, marginRight: 20, marginTop: 15, borderRadius: 20, borderColor: '#FAE7E6' }, { flexDirection: "column" }]}>
        <View style={[styles.bord, { flexDirection: "row" },]}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('AjouterAdmin', donnee)}><Image style={{ marginTop: height * 0.03, marginLeft: width * 0.28, height: height * 0.08, width: width * 0.15 }}
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            <TouchableOpacity onPress={() => {
              if (donnee == item._id) {
                navigation.navigate("ProfileAdmin", item._id)
              }
              else {
                navigation.navigate("ProfileAdmin1", { idadmin: item._id, idUser: donnee })
              }
            }
            }>
              <View style={{ flexDirection: "row" }}>
                ...
                <View>
                  <TouchableOpacity onPress={() => deleteadmin(item._id)}>
                    <Image style={{ marginLeft: width * 0.04, marginTop: height * 0.05 }}
