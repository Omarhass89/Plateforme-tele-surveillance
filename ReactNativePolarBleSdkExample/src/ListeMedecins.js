...
// les médecins qui appartiennent à un hopital donné
...
const ListeMedecins = ({ navigation, route }) => {
  const {hopital,donnee} = route.params;
  ...
  useEffect(() => {
    async function getAllmedecinHs() {
      try {
        const medecinHs = await axios.get(`http://${IP}/api/medecinH/afficher/${hopital._id}`)
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            <TouchableOpacity onPress={() =>
              navigation.navigate("ProfileMedecinH", { profile: item, donnee: hopital })
            }
            >
