...
import { verifyEmailhopital } from './utilsAuth';
...
const VerificationHopital = ({route , navigation}) => {
    ...
    const submitOTP = async () => {
        ...
        if(isObjectValid(OTP)){
            ...
            const res = await verifyEmailhopital(val, profile.id)
            if(!res.success) return updateNotification(setMessage, res.error)
            navigation.dispatch(
                StackActions.replace('ListeHopital',iduser)
            )
        }
...
export default VerificationHopital;