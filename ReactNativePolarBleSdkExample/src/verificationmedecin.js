...
import { verifyEmailmedecin } from './utilsAuth';
...
const VerificationMedecin = ({ navigation , route}) => {
    ...
    const submitOTP = async () => {
        ...
        if(isObjectValid(OTP)){
            ...
            const res = await verifyEmailmedecin(val, profile.id)
            console.log(res)
            if(!res.success) return updateNotification(setMessage, res.error)
            navigation.dispatch(
                StackActions.replace('Doctor',res.medecin.id)
            )
        }
...
export default VerificationMedecin;