 ...
 * @flow strict-local
 */
// Accueil/Details des données du patient : sypmtome, calendrier .. dans le profil de médecin
...
const AccueilPatientH_Med = ({ route }) => {
    ...
    useEffect(() => {
        async function getAllInfoPatient(id) {
            try {
                console.log('ok')
                const info = await axios.get(`http://${IP}/api/patientH/profile/${profilePa._id}`)
                console.log(info)
    ...
    const navigateToQst = () => {
        navigation.navigate('ListeQuiz', { profile: profilePa })
    };
        ...
        <ScrollView>
            <View style={[styles.container, {
                ...
                <View style={{ flexDirection: 'row' }}>
                    ...
                    <TouchableOpacity onPress={navigateToQst} style={{ alignItems: 'center' }} >
                        <View style={{
                            borderRadius: 20,
                            paddingVertical: 14,
                            alignItems: 'center',
                            ...
                            <View alignItems="center">
                                <Image source={require('../images/quest.png')} style={{ width: 0.12 * width, height: 0.1 * height }} />
